...
function runAgentosCommand(command, context) {
  ...
  getChildren(element) {
    ...
    if (element) {
      if (element.label === "Pending Tasks" && this.state.pending_tasks) {
        const taskItems = this.state.pending_tasks.map((t) => new vscode.TreeItem(String(t.description || t.title || t), vscode.TreeItemCollapsibleState.None));
